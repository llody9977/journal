...
2. **ServerHello + Key Share**: Server selects cipher suite (**AES-256-GCM**) and completes the negotiated group's key-establishment step: ordinary Diffie-Hellman for plain X25519, or, for the hybrid group, **ML-KEM encapsulation** against the client's encapsulation key ([RFC 10024](https://www.rfc-editor.org/info/rfc10024/) §4.2) — producing a ciphertext the server sends back alongside its own X25519 share. The server derives the **Handshake Secret** via HKDF from the resulting shared secret; the client derives the same secret by performing **ML-KEM decapsulation** on that ciphertext once it processes ServerHello. The **Main Secret** ([RFC 9846 §7.1](https://www.rfc-editor.org/rfc/rfc9846.html#section-7.1) — called "Master Secret" in the original RFC 8446 wording) is derived immediately afterward, via a further HKDF-Extract step keyed from the Handshake Secret; it does **not** wait for either side's `Finished` message. The server sends its key share.
