...
description: Detailed protocol breakdown of the TLS 1.3 1-RTT handshake, ephemeral and hybrid key establishment, AEAD transport protection, 0-RTT early data replay risks, and Encrypted Client Hello (ECH, RFC 9849).
...
1. **ClientHello + Key Share**: Client sends supported cipher suites, protocol version (TLS 1.3), and an ephemeral key share for its chosen group. That may be a finite-field DH value, an ECDH share such as **X25519**, or the hybrid **X25519MLKEM768** group ([RFC 10024](https://www.rfc-editor.org/info/rfc10024/) §4.1), whose client-side share concatenates a freshly generated ML-KEM-768 **encapsulation key** with an X25519 share — not an ML-KEM ciphertext. Optional ECH is a separate extension carrying an encrypted `ClientHelloInner`; it is not part of `key_share`.
2. **ServerHello + Key Share**: Server selects a cipher suite from the client's offered list and completes the negotiated group's key-establishment step: a finite-field or elliptic-curve Diffie-Hellman response for an ordinary DH group, or **ML-KEM encapsulation** against the client's encapsulation key for X25519MLKEM768 ([RFC 10024](https://www.rfc-editor.org/info/rfc10024/) §4.2), producing a ciphertext sent alongside the server's X25519 share. The server derives the **Handshake Secret** via HKDF from the resulting shared secret; the client derives the same secret by processing the ordinary DH response or decapsulating the hybrid ciphertext. The **Main Secret** ([RFC 9846 §7.1](https://www.rfc-editor.org/rfc/rfc9846.html#section-7.1) — called "Master Secret" in the original RFC 8446 wording) is derived immediately afterward via a further HKDF-Extract step; it does **not** wait for either side's `Finished` message. TLS 1.3 negotiates the AEAD separately, so AES-128-GCM, AES-256-GCM, ChaCha20-Poly1305, and AES-CCM remain independent of the selected key-establishment group.
...
| **TLS 1.3** | **1-RTT** (certificate-authenticated full handshake without a `HelloRetryRequest`; add a round trip if HRR occurs) | **RECOMMENDED STANDARD**: Mandatory AEAD; certificate-authenticated full handshakes use ephemeral finite-field DH, ECDHE, or a standardized hybrid ECDHE+ML-KEM group. PSK-only use without `psk_dhe_ke` — for either resumption or an externally established PSK — remains permitted but forgoes forward secrecy for that connection. Static RSA key transport is absent ([RFC 9846](https://www.rfc-editor.org/rfc/rfc9846.html), which obsoletes the original [RFC 8446](https://www.rfc-editor.org/rfc/rfc8446.html)). | **DEFAULT FOR NEW DEPLOYMENTS**: Use TLS 1.3 by default; retain a restricted TLS 1.2 profile (AEAD-only, ECDHE-only, as described above) only where client compatibility genuinely requires it, and phase that out over time. |
...
- **Client authentication / mTLS**: The server can request a client certificate via `CertificateRequest`; the client responds with its own `Certificate` and `CertificateVerify` inside the client's encrypted post-ServerHello flight described in step 4. Mutual TLS (mTLS) is this mechanism used for service-to-service or zero-trust network authentication, where both directions authenticate via certificates rather than relying solely on server-side authentication plus an application-layer credential.
...
<div class="callout">
  ...
  <p>TLS 1.3 completes a certificate-authenticated full handshake in one round trip without a HelloRetryRequest, mandates AEAD ciphers, and uses ephemeral key establishment for forward secrecy. PSK-only operation is permitted for resumption or external PSKs but forgoes that guarantee. 0-RTT early data is replay-vulnerable and should be restricted to operations explicitly confirmed as replay-safe, not assumed safe from HTTP idempotence alone.</p>
</div>
