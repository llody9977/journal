...
2. **ServerHello + Key Share**: Server selects cipher suite (**AES-256-GCM**), completes Diffie-Hellman, and derives the **handshake secret** via HKDF from that shared secret — the **master secret** (used for application traffic keys) is derived one step later, after the handshake transcript is complete. The server sends its key share.
3. **Encrypted Extensions & Certificate**: All subsequent handshake messages are encrypted using the derived handshake key. Server sends its X.509 certificate and `CertificateVerify` signature, then its own `Finished` message.
4. **Client Finished**: The client verifies the server's handshake transcript, derives its own handshake MAC, and sends its `Finished` message (preceded by the client's `Certificate`/`CertificateVerify` only if the server requested client authentication). Both endpoints now derive the master secret and switch to application traffic keys — this client flight happens *before* ordinary bidirectional application data, per [RFC 9846](https://www.rfc-editor.org/rfc/rfc9846.html).
...
- **Session resumption via tickets**: Rather than repeating a full handshake, TLS 1.3 lets the server issue an opaque `NewSessionTicket` (post-handshake) encapsulating the session's resumption secret. A returning client presents the ticket via the `pre_shared_key` extension, letting both sides skip straight to a PSK-based (optionally PSK+DHE) abbreviated handshake. Tickets should be short-lived and, ideally, single-use or anti-replay-protected server-side, since ticket-based resumption without `psk_dhe_ke` forgoes forward secrecy for that resumed session (see the version matrix above).
...
- **KeyUpdate**: For long-lived connections, either side can send a `KeyUpdate` message to ratchet the traffic keys forward — deriving fresh application traffic keys from the current ones via HKDF — without a full re-handshake. This limits the amount of ciphertext protected under any single key, bounding the impact of a future key compromise and keeping usage within AEAD's per-key data limits on very long connections.
