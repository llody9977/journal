...
The High-Water Mark principle illustrates how FIPS 199 combines per-objective ratings: if a system processes data with Low Confidentiality needs but **High Integrity** requirements (*e.g., flight control software or financial ledgers*), the overall system category is governed by the highest individual rating, not an average—so it would be treated as a **High-Impact System** requiring high-assurance FIPS 200 technical safeguards, under the federal FIPS 199/200 scheme this section describes.
...
Skipping link 4 is the most common failure: a control that was designed and deployed but never verified provides no more assurance than one that was never built, because nothing confirms it behaves as intended under real conditions.
