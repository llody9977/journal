...
<p class="lede">A security objective defines a target property—such as confidentiality, integrity, or availability—that a system is engineered to preserve. Some objectives (confidentiality, integrity) are best understood as invariants a system tries to hold continuously; others (resilience, recovery) are explicitly about how the system behaves and recovers <em>after</em> that invariant is broken, whether the cause is a hostile attacker, an accident, a structural failure, or an environmental event. Technical controls and architectural safeguards are deployed specifically to enforce these objectives. Defining required security properties upfront prevents misapplying cryptographic algorithms, authentication schemes, or backups to solve the wrong failure mode.</p>

<div class="diagram-frame diagram-frame-openable">
  <a class="diagram-open-link" href="{{ '/assets/img/security-objectives-properties-matrix.svg' | relative_url }}" target="_blank" rel="noopener" aria-label="Open the security objectives and system properties matrix at full size">
    <img src="{{ '/assets/img/security-objectives-properties-matrix.svg' | relative_url }}" alt="Security Objectives and System Properties Matrix diagram showing the CIA Triad (FIPS 199), six Extended Security Properties (Accountability, Authenticity, Non-Repudiation, Privacy, Resilience, and Safety), and Technical, Administrative, and Physical Safeguard Alignment. Authenticity mechanisms are shown separately from the identity, key-control, and evidentiary conditions needed to support non-repudiation. The diagram does not depict FIPS 199 Low, Moderate, or High impact severity, which is covered separately in the table below.">
  </a>
  <p class="diagram-caption">Security Objectives &amp; System Properties Matrix: CIA Triad (FIPS 199) → six extended properties (Accountability, Authenticity, Non-Repudiation, Privacy, Resilience, Safety) → Technical, Administrative &amp; Physical Safeguard Alignment (NIST SP 800-53 Rev. 5). Impact-severity categorization (Low/Moderate/High) is covered in the table below, not in this diagram.</p>
</div>

## System Criticality & FIPS 199 / FIPS 200 Impact Categorization

Rather than treating security objectives as arbitrary choices, system architectures evaluate **system criticality** based on potential magnitude of harm. **[NIST FIPS 199](https://csrc.nist.gov/pubs/fips/199/final)** is a U.S. federal standard—mandatory for federal information systems, but its applicability statement explicitly excludes national security systems (defined at 44 U.S.C. § 3552(b)(6); FIPS 199 itself cites the pre-FISMA-2014 location, 44 U.S.C. § 3542(b)(2)) and classified information; those are categorized under a separate national-security-system process (e.g., CNSSI 1253), not FIPS 199. **[NIST SP 800-60 Vol. 1 Rev. 1](https://csrc.nist.gov/pubs/sp/800/60/v1/r1/final)** is supplemental NIST guidance that assists agencies in mapping information types to FIPS 199 categories; it is not itself a mandatory standard, but the Low/Moderate/High impact model they define is a useful, widely borrowed structure for reasoning about system criticality in any organization. Under that model, system categorization follows a 4-step progression:
...
| **Moderate Impact** | **Serious adverse effect**: Significant degradation in operational capability, significant asset damage, or non-fatal injuries. | A B2B SaaS database workload is *often* rated here, but the actual category depends on what the data is and who is harmed if it's breached—not the "B2B SaaS" label itself. | **NIST SP 800-53B Moderate baseline**. The current FedRAMP Rev. 5 path carries the former Moderate assessment baseline into **Class C**, with FedRAMP-defined parameters and requirements layered on top. The class describes assessment information and ongoing commitments, not the cloud service's FIPS 199 category or an agency risk decision; see the [FedRAMP Consolidated Rules for 2026](https://www.fedramp.gov/2026/reference/rev5/c/fedramp-certification/). SOC 2 is an independent attestation framework with its own control criteria—not a NIST Moderate baseline. |
| **High Impact** | **Severe or catastrophic adverse effect**: Complete loss of critical operational capability, major financial destruction, severe injury, or loss of life. | Power grid SCADA control or autonomous vehicle control plausibly reach High on safety grounds; healthcare ePHI or a core banking ledger are not automatically High—the category still depends on the specific harm (e.g., scale, reversibility) the organization assesses. | **NIST SP 800-53B High baseline**. The current FedRAMP Rev. 5 path carries the former High assessment baseline into **Class D**, with additional FedRAMP parameters and requirements; the class itself is not a FIPS 199 category or an agency risk decision. See the [FedRAMP Consolidated Rules for 2026](https://www.fedramp.gov/2026/reference/rev5/d/fedramp-certification/). ISO 26262 is an unrelated automotive functional-safety standard, not a generic FIPS High equivalent—it is cited here only because autonomous vehicle control also falls in the High category by consequence, not because the standards align. |

The high-water-mark process operates at two related levels. FIPS 199 first takes the highest impact for each CIA objective across the information types and functions processed by the system, preserving the resulting three-component system security category. FIPS 200 then takes the highest of those three objective ratings to determine the system's overall impact level for minimum-baseline selection. For example, a system categorized as Low for Confidentiality, High for Integrity, and Low for Availability retains that CIA tuple under FIPS 199, but FIPS 200 treats its overall impact level as High when selecting the minimum baseline—the corresponding SP 800-53B High baseline is a system-wide minimum control baseline, not a set of technical safeguards alone.
...
| **Resilience** | Withstanding active attack, adapting, and recovering core state. | **[NIST SP 800-160 Vol. 2](https://csrc.nist.gov/pubs/sp/800/160/v2/r1/final)** | Automated failover, graceful degradation, and chaos engineering drills. |
| **Safety** | Ensuring failure modes cannot cause physical harm or loss of life. | **[ISO 26262](https://www.iso.org/standard/68383.html)** / **[IEC 61508](https://webstore.iec.ch/publication/5515)** | Fail-safe fault isolation, interlocks, and hazard containment circuits. |
