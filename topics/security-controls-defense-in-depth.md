...
Mitigation Functions (*Preventive, Detective, Corrective, Compensating, Deterrent*) and Control Domains (*Administrative, Physical, Technical*) are independent classification axes. A single mitigation function can be implemented across multiple control domains (*e.g., Preventive access control via administrative policy, physical door locks, or technical WebAuthn tokens*). There is no universal ranking of one domain's effectiveness over another—effectiveness depends on the specific threat, objective, implementation quality, and environment. Each domain instead carries a distinct **consistency and failure-mode profile**:
...
<div class="security-layer security-layer-direct">
  ...
  <div>
    ...
    <ul>
      <li><strong>Control Categories</strong>: Preventive (intercepts known threats before execution), Detective (detects or alerts during or after an event, continuously or periodically), Corrective (restores state—only as clean as its source), Compensating (alternate protection when the primary control is infeasible), Deterrent (discourages an attempt rather than blocking or detecting it).</li>
      ...
      <li><strong>CIS Implementation Groups</strong>: Risk- and resource-based prioritization groups (IG1/IG2/IG3) for which safeguards to adopt first—not a maturity scale.</li>
    </ul>
  </div>
</div>
