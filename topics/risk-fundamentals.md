...
Where the qualitative `Likelihood × Impact` model above is too coarse for financial decision-making, **[FAIR (Factor Analysis of Information Risk)](https://www.fairinstitute.org/what-is-fair)**—an Open Group standard—offers a quantitative alternative: it decomposes risk into loss event frequency and loss magnitude distributions to produce a probable financial loss range (e.g., USD), rather than a Low/Medium/High label.
...
| **Legacy TLS 1.0 Support on Public Web Endpoint** | **CVSS ~5.3** (Medium, hypothetical) | Internet-exposed public endpoint storing financial PII. | **HIGH REGULATORY RISK**: Likely violates **[PCI-DSS 4.0](https://www.pcisecuritystandards.org/)** requirements around strong cryptography for cardholder data. |
