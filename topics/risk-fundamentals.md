...
|---|---|---|
| **Inherent Risk** | Exposure assuming no controls exist at all—the raw combination of asset, threat, and vulnerability. | A theoretical floor; rarely the actual state of a running system, but useful for showing how much a control program is worth. |
...
| **Legacy TLS 1.0 Support on Public Web Endpoint** | **CVSS ~5.3** (Medium, hypothetical) | Internet-exposed public endpoint storing financial PII. | **HIGH REGULATORY RISK**: Likely violates **[PCI DSS 4.0.1](https://www.pcisecuritystandards.org/wp-content/uploads/2024/10/SAQs_for_PCI_DSS_v4.0.1_Bulletin.pdf)** (PCI DSS 4.0 was retired at the end of 2024) requirements around strong cryptography for cardholder data. |
...
|---|---|---|---|
| **Accept** | Formally acknowledge and record residual risk as an authorized decision within the organization's risk appetite/tolerance, made after considering available alternatives—not limited to cases where mitigation cost exceeds impact. | Executive risk sign-off &amp; risk register tracking. | CISO / VP of Engineering sign-off on accepted legacy software risk with 90-day review triggers. |
