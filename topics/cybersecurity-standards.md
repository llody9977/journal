...
description: Technical reference for NIST CSF 2.0, NIST SP 800-53, ISO 27001/27002, OWASP ASVS/SAMM, CIS Controls, CISA CPGs, and SOC 2 audits, and how their altitudes differ.
...
| **[CISA CPGs](https://www.cisa.gov/cross-sector-cybersecurity-performance-goals/cross-sector-cybersecurity-performance-goals)** | Critical Baseline Goals | Minimum high-impact practices for critical infrastructure operators | Critical infrastructure, SMBs establishing a security floor | Voluntary baseline guidance |
| **[FedRAMP](https://www.fedramp.gov/)** | US Federal Cloud Authorization | Authorization framework for cloud service offerings sold to federal agencies, built on NIST SP 800-53 controls | Cloud service providers selling to US federal agencies | Mandatory for in-scope cloud services sold to federal agencies |
...
| **[OWASP Top 10](https://owasp.org/www-project-top-ten/)** | Awareness Document | Consensus list of critical web application security risks | Web developers, application security engineers | Educational awareness floor |
| **[OWASP ASVS 5.0.0](https://owasp.org/www-project-application-security-verification-standard/)** | Verification Standard | Testable application security requirements across 3 levels (L1, L2, L3) | AppSec engineers, penetration testers, QA auditors | Verifiable software standard |
| **[OWASP SAMM v2](https://owaspsamm.org/)** | Maturity Model | Assesses and matures software security practices across 5 business functions | Software engineering leadership | Organizational assessment framework |
| **[SOC 2 (Type I / Type II)](https://www.aicpa-cima.com/resources/landing/aicpa-soc-for-service-organizations)** | Audit Attestation | Independent CPA audit report based on AICPA Trust Services Criteria | B2B SaaS vendors proving control operation to enterprise buyers | Independent CPA Audit Attestation Report |
...
- **OWASP ASVS**: Concrete, testable verification requirements at three levels. ASVS 5.0 replaced the data-sensitivity tiers used in ASVS 4.0 with a priority-based split, so the level names from the older edition (*Opportunistic*, *Standard*, *High Assurance*) no longer apply:
  - **Level 1**: the minimum set — roughly 20% of ASVS requirements, deliberately small to keep the barrier to entry low. Mostly critical first-layer defenses against attacks that need no other precondition. ASVS notes that Level 1 is *not* necessarily verifiable by an external black-box tester without access to documentation or code.
  - **Level 2**: the level most applications should target — about 50% of requirements, so roughly 70% of the standard cumulatively. Covers less common attacks and more complex protections against common ones.
  - **Level 3**: the remaining ~30% — largely defense-in-depth mechanisms and controls that are useful but hard to implement.
- **OWASP SAMM**: Assesses and matures an organization's software security program across 5 business functions comprising 15 security practices — governance and design as well as build and release.

## CIS Controls v8.1: Prioritized Implementation

For organizations seeking a direct, prioritized roadmap, **[CIS Controls v8.1](https://www.cisecurity.org/controls/cis-controls-list)** organizes 153 Safeguards into three **Implementation Groups (IGs)**:
...
Enterprise B2B SaaS buyers commonly ask for a SOC 2 Type II report as contractual evidence that controls actually operated. It is not the only accepted form: ISO 27001 certification is a frequent alternative, a Type I is sometimes accepted for a vendor's first year, and which evidence satisfies a buyer is a procurement decision rather than a fixed rule.

### Reading a SOC 2 report rather than collecting it

A report's existence is weaker evidence than its contents. Four things determine what it actually proves:

- **Scope**: which systems, products, and Trust Services Criteria categories the engagement covered. A report scoped to one product line says nothing about another.
- **Observation period**: a Type II covers a stated window. A report whose period ended fourteen months ago is stale, and gaps between consecutive periods are not covered by either report.
- **Opinion**: qualified or unqualified. A qualified opinion names the criteria the auditor could not conclude on.
- **Exceptions and CUECs**: tested exceptions appear in the results section, and complementary user entity controls list what the report assumes *you* operate. Controls assigned to the customer are outside the vendor's assurance.
...
| **[NIST SP 800-53 Rev. 5](https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final)** | Control Catalog &amp; Safeguards | Catalog of 20 control families (*Access Control, Audit and Accountability, System and Communications Protection, Incident Response*). There is no separate Cryptography family — cryptographic controls sit inside System and Communications Protection (SC). | **Enterprise Governance &amp; Control Execution**: The primary baseline catalog for FedRAMP, DoD, and enterprise risk controls. |
...
| **[NIST SP 800-160 Vol. 1](https://csrc.nist.gov/pubs/sp/800/160/v1/r1/final)** / **[Vol. 2](https://csrc.nist.gov/pubs/sp/800/160/v2/r1/final)** | System Security Engineering | Systems security engineering for trustworthy secure systems, aligned to the ISO/IEC/IEEE 15288 lifecycle processes (Vol. 1); cyber-resiliency design principles for systems expected to keep operating while compromised (Vol. 2) | **System Security Engineering**: Building security into system design rather than adding it afterwards, and designing for continued operation under adversary presence. |
...
| **[DISA STIGs](https://www.cyber.mil/stigs/)** | Defense &amp; Federal System Hardening | DoD mandatory security technical implementation guides | **High-Assurance Federal Infrastructure**: Mandatory technical configuration baselines for military and defense IT environments. |

## Which one to reach for

The lede's question — which model an organization actually needs — resolves by asking what the output has to be, not which standard is best:

| If the need is… | Reach for | Because |
|---|---|---|
| A shared vocabulary for reporting posture to executives or a board | **NIST CSF 2.0** | It defines outcomes, not implementations, so it survives changes of tooling and maps onto whatever catalog sits beneath it. |
| A prioritized list of things to actually do next, with limited staff | **CIS Controls IG1** | 56 safeguards chosen for the attacks that occur most, deliverable without a dedicated security team. |
| An exhaustive control catalog to select a baseline from | **NIST SP 800-53** | Required for federal systems via FIPS 200; useful elsewhere precisely because it is exhaustive rather than prioritized. |
...
<div class="callout">
  ...
  <p>ISO 27001 defines a management system, NIST SP 800-53 provides a technical control catalog, and hardening baselines like CIS Benchmarks give prescriptive per-system configuration — these operate at different altitudes and aren't interchangeable substitutes. Pick one framework as the organizing spine, map the rest into it, and choose by what output is required rather than by which standard is most cited.</p>
</div>
...
- **NIST Cybersecurity Framework 2.0**: *CSWP 29* — [NIST CSF 2.0](https://doi.org/10.6028/NIST.CSWP.29) — verified the six Functions and the February 2024 publication.
- **OWASP ASVS 5.0**: *Application Security Verification Standard* — [OWASP ASVS](https://owasp.org/www-project-application-security-verification-standard/) — verified the three priority-based levels and their proportions.
- **AICPA**: *SOC for Service Organizations and the Trust Services Criteria* — [AICPA SOC](https://www.aicpa-cima.com/resources/landing/aicpa-soc-for-service-organizations) — verified the five TSC categories and the Type I / Type II distinction.
- **CIS Critical Security Controls v8.1** — [CIS Controls list](https://www.cisecurity.org/controls/cis-controls-list) — verified 18 Controls, 153 Safeguards, and the Implementation Group structure.
