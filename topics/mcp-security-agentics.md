...
Excessive agency — an agent holding capability beyond what its task requires — is **LLM03:2026** in the [OWASP Top 10 for LLM Applications](https://genai.owasp.org/resource/owasp-genai-llm-top-10-2026/), climbing three places from `LLM06:2025` — the move the project itself calls the most consequential on the list, because the community vote and the incident record agree that agentic deployments are where the damage lands. The specification's position is a **SHOULD**: there should always be a human in the loop able to deny a tool invocation, and applications should show which tools are exposed, indicate when they are invoked, and present confirmation prompts.
