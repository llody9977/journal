...
- **Probabilistic vs. deterministic finality**: Nakamoto-style PoW chains (Bitcoin) never give absolute finality — every confirmation only reduces the probability of a later reorg, asymptotically, and a sufficiently resourced attacker can in principle still reverse many confirmations. BFT-style finality gadgets (Ethereum's Casper FFG, Tendermint-based chains) instead provide **deterministic finality** in protocols that implement slashing: once a block is finalized under the protocol's rules, reverting it requires validators controlling at least the protocol's slashing threshold to provably violate the protocol and be slashed. That threshold is a per-protocol design choice, not a universal constant — Ethereum's Casper FFG and classic Tendermint BFT both use roughly 1/3 of stake as their safety threshold, but other BFT-style designs choose different fractions, and plenty of chains (Bitcoin's Nakamoto consensus among them) have no slashing mechanism or deterministic finality at all. Either way, this is a fundamentally different guarantee than "sufficiently many confirmations," with different assumptions about validator honesty and network synchrony.
...
- **Bridge and oracle risks**: Cross-chain bridges and price/data oracles sit outside the cryptographic guarantees of any single chain — a bridge's security reduces to whatever mechanism it uses to attest that an event happened on the source chain (a federated multisig, a light-client proof, an optimistic fraud-proof window), and that mechanism's trust assumptions are frequently much weaker than the underlying chains' consensus security. Bridges and oracles have repeatedly been among the largest crypto exploits by dollar value in incident retrospectives from blockchain security firms — the underlying reason is structural, not coincidental: they concentrate value while inheriting security from whatever mechanism attests to off-chain or cross-chain state, which is frequently weaker than the base-layer consensus and cryptography this page covers.
