...
This makes silent tampering computationally impractical to conceal, but "immutability" here is probabilistic, not absolute — and the threshold for rewriting history is protocol-specific, not one universal number. In **Nakamoto-style PoW** (Bitcoin), an attacker controlling a majority of hashpower can, given enough time, outrun the honest chain and rebuild an alternative history from a past point — the classic "51% attack." **PoS protocols vary far more**, and this is where the page's later distinction between probabilistic and deterministic finality (below) actually matters here: a PoS chain using BFT-style deterministic finality (Casper FFG, Tendermint) cannot have its already-finalized blocks rewritten by a majority-stake attacker who stays under the protocol's specific safety threshold — doing so requires crossing that threshold and triggers slashing, not simply accumulating "a majority." Weakly-subjective PoS designs, chains without deterministic finality, and the *unfinalized* tail of any chain remain more analogous to the PoW case. Communities have, in extreme cases, chosen to fork away from an already-published chain regardless of the underlying consensus mechanism (e.g., Ethereum's 2016 DAO fork, a PoW-era social/hard-fork decision, not a cryptographic rewrite). Deeper confirmation depth or higher finality thresholds raise the cost of rewriting history; they don't make rewriting mathematically impossible.
...
- **Key custody is the whole system's actual failure mode in practice**: none of the cryptography on this page protects funds if the private key controlling them is lost or stolen — there is no password reset, no customer support override, and no chain-level mechanism to reverse a transaction signed by a stolen key (that would require the same kind of majority/finality-threshold rewrite discussed above, which legitimate custody failures don't trigger). This is why serious custody design doesn't rest on a single private key: **multisignature** (M-of-N independent keys must all sign) and **threshold signature schemes** (a single signature is produced collaboratively from N key shares, with no single share ever reconstructing the full key) both remove any one compromised key or device as a single point of failure. Beyond the signing scheme itself, production custody needs a **key rotation** plan (moving funds to a new key set on a schedule or after a suspected exposure), a **recovery** plan for lost key shares that doesn't itself become a backdoor, and **emergency controls** (a pre-authorized path to freeze or migrate funds quickly if a compromise is detected) — all of which are operational/systems design, not something the underlying signature scheme provides automatically just by existing.
