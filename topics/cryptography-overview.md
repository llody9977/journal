...
- **Entropy sources**: Operating systems pool unpredictability from hardware timing jitter (disk seek times, keyboard/mouse interrupt timing, network packet arrival jitter), dedicated hardware RNGs (Intel `RDRAND`/`RDSEED`, ARM's `RNDR`, TPM-based RNGs), and — where available — hardware noise sources built into the CPU. Linux's `getrandom()` syscall and `/dev/urandom` draw from this pooled entropy; on modern kernels both block only until the pool is initially seeded at boot, not on every call. **Virtualized and embedded environments are the weak case**: a VM or container has fewer physical timing sources to draw jitter from, and early-boot entropy starvation is a real, well-documented failure mode (e.g., cloud instances or IoT devices generating predictable keys moments after first boot, before the entropy pool has accumulated enough unpredictability).
...
- **Reseeding**: SP 800-90A DRBGs specify a `reseed_interval` — a maximum number of requests (or bytes) a DRBG instance may serve before it must draw fresh entropy and reseed. This bounds the damage from an eventual internal-state compromise (see Backtracking Resistance, above) and defends against the DRBG's internal state drifting into a low-entropy cycle over a very long operational lifetime. Applications using a platform CSPRNG (`crypto.getRandomValues()`, `os.urandom()`, `SecureRandom`) get this handled transparently by the OS; applications that instantiate and hold their own long-lived DRBG object need to reseed it themselves on the platform's schedule.
- **Process forks**: `fork()` duplicates the entire parent process's memory, including any in-process DRBG's internal state — if both the parent and child then generate "random" output from that duplicated state without an intervening reseed, they produce **identical output streams**, silently breaking key/nonce uniqueness across the two processes. This is a real, recurring class of bug (server workers forked from a shared master process, each expected to generate independent session keys or nonces). The fix is fork-awareness: glibc and OpenSSL register post-fork hooks that force a reseed, and Python's `os.register_at_fork()` exists specifically so application code can do the same for any userspace DRBG state it holds; using the OS-level CSPRNG call (`getrandom()`, `/dev/urandom`) on every request rather than caching a DRBG instance sidesteps the problem entirely, since the kernel's own state isn't duplicated the same way user-space state is.
- **VM/container snapshot cloning**: Cloning a running VM or container snapshot duplicates its entire memory image at the instant of the snapshot — including CSPRNG internal state — the same way `fork()` does, but at a coarser granularity and often with a much longer gap before anyone notices. Booting multiple instances from the same golden snapshot without forcing each one to reseed from a fresh, instance-specific entropy source has caused real-world key-reuse incidents; cloud platforms and hypervisors that support VM cloning generally provide (and application images should invoke) an explicit "reseed on clone" mechanism rather than assuming the guest OS detects the clone on its own.
...
<div class="security-layer security-layer-direct">
  ...
  <div>
    ...
    <ul>
      ...
      <li><strong>DRBG State Hazards</strong>: <code>fork()</code> and VM/container snapshot cloning both duplicate in-process CSPRNG state — reseed explicitly after either event, or rely on a fresh OS-level CSPRNG call per use instead of a long-lived DRBG instance.</li>
