...
- **Process forks**: `fork()` duplicates the entire parent process's memory, including any in-process DRBG's internal state — if both the parent and child then generate "random" output from that duplicated state without an intervening reseed, they produce **identical output streams**, silently breaking key/nonce uniqueness across the two processes. This is a real, recurring class of bug (server workers forked from a shared master process, each expected to generate independent session keys or nonces). Whether a given library reseeds automatically on fork is **implementation- and version-specific, not something to assume** — it has changed over time even within widely used libraries (OpenSSL's fork-handling APIs, for instance, have gone through multiple design iterations; see [OpenSSL's `OPENSSL_fork_prepare` documentation](https://docs.openssl.org/3.2/man3/OPENSSL_fork_prepare/) for the current mechanism and don't assume older guidance still applies to your linked version). Python's `os.register_at_fork()` is a stdlib primitive that lets application code register its own post-fork reseed logic for any userspace DRBG state it holds, which is more reliable than assuming a library handles it for you. The dependency-free fix is to avoid the question entirely: call the OS-level CSPRNG (`getrandom()`, `/dev/urandom`) fresh on every use rather than caching a DRBG instance, since the kernel's own entropy state isn't duplicated across `fork()` the way user-space state is.
