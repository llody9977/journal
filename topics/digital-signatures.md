...
| **Ed25519** ([RFC 8032](https://www.rfc-editor.org/rfc/rfc8032) §5.1) | Edwards-curve Digital Signatures | **SECURE BY DESIGN**: Nonce derived deterministically via SHA-512(private key half \|\| message) — no per-signature RNG dependency. | Standard default for modern APIs, SSH keys, WebAuthn, and software signing. |
| **Ed448** ([RFC 8032](https://www.rfc-editor.org/rfc/rfc8032) §5.2) | Edwards-curve Digital Signatures | **SECURE BY DESIGN**: Nonce derived deterministically via SHAKE256(private key half \|\| message) — no per-signature RNG dependency. | Higher security-level alternative to Ed25519; less common support in SSH/WebAuthn tooling, useful for long-term high-assurance roots. |
