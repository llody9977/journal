...
|---|---|---|---|
| **Attack Surface** | The aggregate set of reachable endpoints, open ports, public APIs, user inputs, identities and credentials, administrative interfaces, outbound flows, local/physical interfaces, and dependencies where an adversary can attempt entry or a failure can propagate. | Minimizing total exposed exposure area across network, identity, and supply-chain vectors—not network reachability alone. | External vulnerability scanning, port audits, attack surface management (ASM) &amp; software supply-chain (SBOM) review. |
...
| **Stage 3: Mitigation & Control Selection** | Select and implement technical security controls that reduce the likelihood or impact of identified high-risk threats—controls lower risk, they do not universally neutralize it. | NIST SP 800-53 / OWASP ASVS 5.0.0 | Applied security controls (WAF, mTLS, WebAuthn, AES-256). |
| **Stage 4: Retrospective & Verification** | Validate coverage of scope, assumptions, and scenarios; confirm controls are actually implemented as designed; and record residual risk—automated testing (SAST/DAST) checks implementation but does not by itself confirm scope or assumption completeness. | OWASP Question 4 &amp; NIST SP 800-137 | Verification record covering scope/assumption review, automated SAST/DAST regression tests, and threat model sign-off with documented residual risk. |
