...
- **Build L1**: Requires provenance to exist and be distributed, generated by a hosted build platform rather than an individual workstation — it does not require the build itself to run from an automated script (that producer requirement was removed in the v1.0 specification).
- **Build L2**: Requires the provenance be authentic and verifiable; SLSA recommends, but does not mandate, a digital signature as the mechanism.
