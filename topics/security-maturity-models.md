...
- **Build L1**: Requires provenance — an automated build script generates provenance data detailing build sources.
- **Build L2**: Requires signed provenance generated by a hosted build platform.
- **Build L3**: Adds a hardened build platform that enforces hermetic build isolation and protects signing keys against tampering.
