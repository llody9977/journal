...
3. **Linear Scaling of ECC**: Elliptic Curve Discrete Logarithms (ECDLP) have no sub-exponential attack algorithm — the best generic attack (Pollard's rho) costs roughly 2<sup>n/2</sup> for an n-bit curve group, so security strength scales roughly linearly with curve size, at about half the curve's bit length. This is the mental model behind the table above (224 → 256 → 384 → 521 bits); the specific NIST-approved curve sizes are discrete parameter choices dictated by [NIST SP 800-57 Part 1 Rev. 5](https://csrc.nist.gov/pubs/sp/800/57/pt1/r5/final)'s strength ranges, not an exact doubling sequence.
