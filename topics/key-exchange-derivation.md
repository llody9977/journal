...
<div class="security-layer security-layer-direct">
  ...
  <div>
    ...
    <ol>
      ...
      <li><strong>Identical Shared Secret &amp; Secret Lifecycle</strong>: Both sides arrive at the exact same shared secret <b>S</b>. Neither side uses <b>S</b> directly as a key — a protocol passes it through a KDF such as HKDF to derive context-specific traffic keys and IVs. Security-conscious native implementations should erase superseded ephemeral secrets where their language, runtime, and platform permit it, but the protocol does not guarantee erasure and the exact behavior must be verified. Swap, hibernation, crash dumps, managed runtimes, and implementation defects can preserve secret material after its intended lifetime.</li>
    </ol>
  </div>
</div>
...
- **Resistance to offline guessing**: A PAKE's core security property is that the protocol transcript an eavesdropper observes gives no information usable for offline password guessing — every guess an attacker makes has to be checked interactively against the live server (which can rate-limit or lock out repeated failures), not verified silently against captured data. This is a guarantee about a specific attacker state — a *passive network observer* of the live exchange — and it's worth not conflating with a different attacker state: "password sent over TLS" already protects the password from that same passive network observer during transmission, but that's a transport-layer property, separate from what happens to the password *after* it reaches the server. Once stored, an ordinary salted-hash verifier is directly offline-guessable if the credential store is later breached (see the Password Storage page) — a *stored-credential compromise*, not a network-observation one. A PAKE's passive-transcript protection doesn't by itself say anything about that separate, later risk; whether the stored server-side data also resists offline guessing after a breach depends on the specific PAKE variant (see the augmented-PAKE registration-record discussion below, including its own limits under full server compromise).
- **Server registration records**: For an augmented PAKE, the server-side record is deliberately not the password and not a simple salted hash of it. Unlike a conventional salted password hash — which by itself is enough to mount an offline guessing attack — the OPAQUE registration record alone is insufficient for that; the relevant server-side OPRF secrets are also required (see the Password Storage page for how salted-hash storage compares). But that's narrower protection than it sounds, for two reasons. First, [RFC 9807 itself is explicit](https://www.rfc-editor.org/rfc/rfc9807.html) that offline dictionary attacks are not eliminated by OPAQUE, only made harder: a server compromise that yields both the stored registration record and any server-side secrets needed to use it still lets an attacker run an offline guessing attack against the password. Second, if that offline attack succeeds, the attacker recovers the actual password — a credential that *is* directly reusable anywhere the user reused it, so the registration record's non-reusability doesn't survive a successful password recovery. A key-stretching function (KSF, e.g. Argon2id) raises the cost per guess, the same role it plays in ordinary password storage, but it doesn't make a low-entropy password's recovery infeasible; PAKE's offline-guessing resistance is specifically about the live network protocol exchange, not a guarantee that survives full server compromise.
...
|---|---|---|
| **Impact of Private Key Leak** | In deployments that use the leaked long-term key to transport or derive session keying material — static RSA key transport or static, non-ephemeral Diffie-Hellman — the adversary can decrypt recorded historical traffic protected under that key when the required transcript or ciphertext was captured. | An adversary **cannot derive past session keys from the leaked long-term authentication key alone**; recorded sessions remain protected unless the ephemeral key-establishment secrets or another required input were independently compromised. |
| **Key-Establishment Mechanics** | RSA key transport or static Diffie-Hellman | Fresh finite-field DH, ECDHE/X25519, or an ephemeral hybrid group such as X25519MLKEM768 |
| **Modern Protocol Boundary** | Static RSA key transport and static DH are absent from **TLS 1.3** ([RFC 9846](https://www.rfc-editor.org/rfc/rfc9846.html), which obsoletes the original [RFC 8446](https://www.rfc-editor.org/rfc/rfc8446)). | Certificate-authenticated TLS 1.3 full handshakes use ephemeral key establishment; standardized groups include finite-field DH, ECDHE, and hybrid ECDHE+ML-KEM. TLS 1.3 also permits PSK-only operation without `psk_dhe_ke`, which forgoes forward secrecy for that connection. SSHv2 similarly favors ephemeral ECDH/DH methods, but [RFC 9142](https://www.rfc-editor.org/rfc/rfc9142.html) defines per-algorithm implementation tiers rather than one universal ephemeral-only rule. |

<div class="security-layer security-layer-protect">
  <div class="security-layer-label">Architectural Roles</div>
  <div>
    <strong>Server Authentication Key vs. Ephemeral Key-Establishment Secrets</strong>
    ...
    <ul>
      ...
      <li><strong>Ephemeral Key-Establishment Secrets (Intended to Stay in Memory Only)</strong>: A certificate-authenticated TLS 1.3 full handshake uses fresh finite-field DH, ECDHE/X25519, or a hybrid ECDHE+ML-KEM group to establish shared secret material. Those values are key-establishment inputs, not traffic-encryption keys themselves. The TLS HKDF key schedule derives the negotiated AEAD traffic keys and IVs. Superseded secrets are intended to be discarded, but erasure is an implementation goal rather than a protocol guarantee; swap, hibernation, managed runtimes, or crash dumps can preserve them.</li>
      <li><strong>Client Certificates</strong>: In ordinary web browsing, clients normally do not present certificates. The browser still contributes ephemeral key-establishment material and derives traffic secrets through the TLS key schedule; server authentication and client authentication are separate from that confidentiality mechanism.</li>
    </ul>
  </div>
</div>
      ...
      if (activeMode === 'rsa') {
        ...
        <div class="security-layer security-layer-direct" style="margin-top: 1rem;">
          ...
          <div>
            ...
            <p style="margin-bottom:0;">This branch doesn't attempt decryption the way the Static RSA branch above does — there is nothing here for the stolen authentication key to decrypt with, which is precisely the property being demonstrated. Recomputing either session's symmetric traffic key would require the client and server's ephemeral ECDH private keys, which this demo never persisted anywhere the attacker's stolen key can reach. Security-conscious native implementations should erase superseded ephemeral secrets where the language, runtime, and platform permit it, but that behavior must be verified for the exact implementation. JavaScript itself offers no guaranteed secure-erasure primitive.</p>
          </div>
        </div>`;
      }
      state.step = 5;
