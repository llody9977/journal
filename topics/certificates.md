...
A leaf certificate (also called an **end-entity certificate**) is issued to a website, person, device, or application rather than another CA. It cannot act as a CA. Basic Constraints may explicitly say `CA:FALSE`; under [RFC 5280](https://www.rfc-editor.org/rfc/rfc5280), an absent extension also means the certificate is not a CA. Certification-path validation additionally checks key usage, constraints, and the issuer chain.
...
A **CSR** is an encoded PKCS#10 structure, defined by [RFC 2986](https://www.rfc-editor.org/rfc/rfc2986), submitted to a CA to request a signed certificate. It includes:
...
- **Proof of Possession (POP):** A digital signature created with the applicant's private key over the CSR data. Verifying it shows that the requester can use the matching private key; it does not by itself prove who the requester is or where the key is stored.
...
- **OCSP (Online Certificate Status Protocol)** — [RFC 6960](https://www.rfc-editor.org/rfc/rfc6960) defines how a client asks an OCSP responder for the status of one certificate. Direct client queries can expose browsing behavior to the responder and add a live dependency and latency to the connection.
...
- Automate issuance and renewal with [ACME, RFC 8555](https://www.rfc-editor.org/rfc/rfc8555) (for example, Let's Encrypt or an ACME-compatible private CA). Manual renewal becomes increasingly fragile as certificate lifetimes shrink.
...
<div class="callout">
  <span class="callout-title">RFC map</span>
  <p><strong><a href="https://www.rfc-editor.org/rfc/rfc5280">RFC 5280</a></strong> defines the Internet X.509 certificate and CRL profile. <strong><a href="https://www.rfc-editor.org/rfc/rfc2986">RFC 2986</a></strong> defines PKCS #10 certificate requests. <strong><a href="https://www.rfc-editor.org/rfc/rfc6960">RFC 6960</a></strong> defines OCSP. <strong><a href="https://www.rfc-editor.org/rfc/rfc8555">RFC 8555</a></strong> defines ACME certificate automation. <strong><a href="https://www.rfc-editor.org/rfc/rfc9162">RFC 9162</a></strong> defines Certificate Transparency version 2; browser programs apply their own current CT policies on top.</p>
</div>
