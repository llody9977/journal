...
- **OCSP Stapling**: The server itself periodically fetches a signed OCSP response from the CA and "staples" it to the TLS handshake, so the client doesn't need its own round-trip to the CA. This fixes the metadata-leak and latency problems, but only works if the server actually keeps its stapled response current. What happens on a missing or expired staple isn't one uniform behavior — it varies by client and policy: an ordinary client with no specific expectation typically proceeds anyway (functionally soft-failing, the same gap as plain OCSP), but a certificate carrying the TLS Feature extension (**"Must-Staple," [RFC 7633](https://www.rfc-editor.org/rfc/rfc7633.html)**) signals that clients supporting the extension should hard-fail the connection if no valid staple is provided — turning a missing staple into a real rejection for those clients specifically, at the cost of the server now needing to guarantee staple availability.
...
<div class="security-layer security-layer-direct">
  ...
  <div>
    ...
    <ul>
      <li><strong>Web Browsers (Deprecated)</strong>: <strong>HTTP Public Key Pinning (HPKP / RFC 7469)</strong> was officially <strong>deprecated and removed from Chrome and Firefox</strong> due to site-bricking hazards and malicious pin-jacking attacks — Safari never implemented HPKP in the first place, so it wasn't part of that removal. Web browsers rely on <strong>Certificate Transparency (CT)</strong> for rogue CA detection instead.</li>
