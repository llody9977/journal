...
<div class="diagram-frame diagram-frame-openable">
  <a class="diagram-open-link" href="{{ '/assets/img/pqc-primitive-selection.svg' | relative_url }}" target="_blank" rel="noopener" aria-label="Open the quantum impact by primitive class diagram at full size">
    <img src="{{ '/assets/img/pqc-primitive-selection.svg' | relative_url }}" alt="Three rows giving the applicable quantum algorithm, its ideal query bound, and the resulting selection decision per primitive class. Symmetric ciphers face Grover at about 2^(n/2) sequential queries, so retain the cipher and pick key size from the governing profile. Hash functions face Grover for preimages at about 2^(n/2) and BHT for collisions at about 2^(n/3) under strong quantum-memory assumptions, so select digest size from the required property. Public-key primitives face Shor in polynomial time, an asymptotic break, so migrate to FIPS 203 ML-KEM and FIPS 204 ML-DSA.">
  </a>
  <p class="diagram-caption">Only the public-key row implies replacing the algorithm; for symmetric ciphers and hashes, Grover and BHT change the size decision rather than the choice — and all three exponents are ideal query bounds, not concrete attack budgets</p>
</div>
