...
2. **Shor's Algorithm (Asymmetric Public Key Cryptography)**: Solves prime factorization and discrete logarithms in polynomial time — commonly cited as **O(n^3)**, though the precise exponent depends on the circuit/arithmetic model and isn't a single settled constant — versus the best-known classical algorithms' sub-exponential running time. This would render RSA, ECC, ECDSA, and Diffie-Hellman tractable to break, contingent on a sufficiently capable, fault-tolerant quantum computer actually running it, which does not exist today; [NIST's own PQC evaluation criteria](https://csrc.nist.gov/Projects/Post-Quantum-Cryptography/Post-Quantum-Cryptography-Standardization/Evaluation-Criteria/Security-%28Evaluation-Criteria%29) caution that concrete quantum security estimates are more nuanced than headline asymptotic figures. Mitigated by transitioning to NIST Post-Quantum Cryptography (PQC) standards ahead of that threshold.
