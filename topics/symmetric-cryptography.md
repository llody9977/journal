...
<p class="lede">Symmetric cryptography is the oldest and simplest idea in this entire field: one secret, shared by both sides, does both the locking and the unlocking. It's also, in production systems today, doing almost all of the actual heavy lifting — every other primitive covered elsewhere exists largely to get a symmetric key safely into place.</p>
...
- **No non-repudiation** — because both sides hold the identical key, either one could have produced any given ciphertext or MAC; there's no way to prove which one actually did. This isn't a quality-of-implementation gap, it's structural — see [Symmetric vs Asymmetric Cryptography]({{ '/topics/symmetric-vs-asymmetric/' | relative_url }}#non-repudiation-the-one-guarantee-only-asymmetric-crypto-can-offer) for why only asymmetric crypto can offer this at all.
...
<div class="callout warn">
  ...
  <p>CTR and GCM both depend on a nonce (a number used once) that must never repeat under the same key. Reuse a nonce in CTR mode and the keystream repeats, letting an attacker XOR two ciphertexts together to cancel it out and recover information about both plaintexts. Reuse a nonce in GCM and it's worse — it can lead to full recovery of the authentication key, breaking integrity for every message ever sent under that key. This is almost always the actual root cause when "AES was broken" turns out to be reported in the news.</p>
</div>
...
## Practical demo: encrypting a file with OpenSSL

```
$ openssl enc -aes-256-gcm -pbkdf2 -salt \
    -in secret.txt -out secret.txt.enc -pass pass:correct-horse-battery-staple

$ ls -la secret.txt.enc
-rw-r--r--  1 user  staff  156 Jul 25 10:02 secret.txt.enc
```
...
$ openssl enc -d -aes-256-gcm -pbkdf2 \
    -in secret.txt.enc -out secret-decrypted.txt -pass pass:correct-horse-battery-staple

$ diff secret.txt secret-decrypted.txt
# (no output — files are identical)
```

`-pbkdf2` derives the actual AES key from the passphrase (see [Key Exchange & Key Derivation]({{ '/topics/key-exchange-derivation/' | relative_url }}) for why you never use a raw password as a key directly). In a real system, the key more commonly comes from a key management service or a prior key exchange, not a hardcoded passphrase on a command line.
...
- **Never use a raw human passphrase as the key directly.** A passphrase has far less real entropy than its character length suggests, and is brute-forceable at the *passphrase* level even when AES itself isn't remotely threatened. This is exactly why the OpenSSL demo above uses `-pbkdf2` rather than hashing the passphrase directly into a key — see [Key Exchange & Key Derivation]({{ '/topics/key-exchange-derivation/' | relative_url }}#a-different-problem-password-based-kdfs) and [Password Storage]({{ '/topics/password-storage/' | relative_url }}) for the full reasoning.
- **Rotate keys to limit the blast radius, not to make brute force harder.** A single key's strength doesn't degrade with age or use — rotation exists so that if a key is ever compromised, only the data encrypted under that specific key (not everything ever encrypted) is exposed. [HSM & KMS]({{ '/topics/hsm-kms/' | relative_url }}#key-rotation-what-actually-happens-and-why-it-doesnt-touch-the-data) covers exactly how rotation works underneath a real key-management system.

## Common pitfalls

- **Using ECB** — still shows up in default settings of older libraries and misconfigured tools. Always check the mode explicitly.
- **Reusing a nonce/IV** under the same key — catastrophic for CTR/GCM, serious for CBC too.
